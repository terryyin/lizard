...
"""
source_analyzer (verion 1.5) is a simple code complexity source_file_counter without caring about the C/C++ header files.
...
def analyze(paths, exclude_patterns, use_preprocessor=None, threads=1):
    ...
    return itertools.imap(analyze_source_file, iterate_files(paths, exclude_patterns), itertools.repeat(use_preprocessor))
...
def main(argv):
    ...
    if options.working_threads > 1:
        ...
        r = itertools.imap(analyze_source_file, files, itertools.repeat(options.use_preprocessor))
    if options.xml:
