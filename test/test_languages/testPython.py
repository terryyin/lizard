...
class Test_Python_pep695_type_parameters(unittest.TestCase):
    """PEP 695 type params must not overwrite the function name.

    `def f[T](x)` was reported as a function named ']', because every token
    between `def` and `(` restarted the function.
    """

    def test_generic_function_keeps_its_name(self):
        code = (
    ...
    def test_default_typevartuple_and_paramspec(self):
        for code in (
            ...
            functions = get_python_function_list(code)
            self.assertEqual('f', functions[0].name, code)
    ...
    def test_constraints_and_nested_callable_bound(self):
        for code in (
            ...
            functions = get_python_function_list(code)
            self.assertEqual('f', functions[0].name, code)
            self.assertEqual(1, functions[0].parameter_count, code)
    ...
    def test_long_name_is_not_corrupted(self):
        code = (
            'def soma[T](a: T, b: T) -> T:\n'
            '    return a\n'
        )
        functions = get_python_function_list(code)
        self.assertTrue(functions[0].long_name.startswith('soma('),
                        functions[0].long_name)
